...
describe Facter::TPM2::Util do
  ...
  context 'when tpm2-tools can query the TABRM' do
    ...
    it 'should return a correct data structure queried from the TPM of any manufacturer' do
      ...
      allow(Facter::Core::Execution).to receive(:execute).with("/usr/bin/tpm2_getcap -c properties-variable").and_return(variable_yaml_string)
      ...
      yaml_strings.each do |yaml_string|
        allow(Facter::Core::Execution).to receive(:execute).with("/usr/bin/tpm2_getcap -c properties-fixed").and_return( yaml_string )
  ...
  context 'when tpm2-tools cannot query the TABRM' do
    it 'should not raise a failure' do
      ...
      allow(Facter::Core::Execution).to receive(:execute).with("/usr/bin/tpm2_getcap -c properties-variable").and_return("")
