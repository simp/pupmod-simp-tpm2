...
3. **Optionally takes/changes ownership of the TPM** — sets or clears the
   ...
   `tpm2::take_ownership` is `true` (`manifests/init.pp:44-46`).
...
- **`tpm2` (`manifests/init.pp:25-48`)** — public entry class; consumers
  `include 'tpm2'`. Parameters (`init.pp:26-30`):
  - `$package_ensure` (`String[1]`) — defaults to
    `simplib::lookup('simp_options::package_ensure', {'default_value' => 'installed'})`
    (`init.pp:26`). This is the **only** `simp_options::*` seam in the module.
  - `$packages` (`Hash[String[1], Hash[String[1],String[1]]]`) —
    `simplib::lookup('tpm2::packages')` with **no default_value**
    (`init.pp:27`), so it is resolved entirely from module data
    (`data/common.yaml`). A missing `tpm2::packages` key is a compile error.
  - `$tabrm_service` (`String[1]`) — default `'tpm2-abrmd'` (`init.pp:28`).
  - `$tabrm_options` (`Optional[Array[String[1]]]`) — default `undef`; an
    **explicitly unvalidated** list of options passed to the service at start
    (`init.pp:20-21,29`).
  - `$take_ownership` (`Boolean`) — default `false` (`init.pp:30`).

  Control flow (`init.pp:32-47`):
  - `simplib::assert_metadata($module_name)` runs first (`init.pp:32`).
  - **tpm1 guard** (`init.pp:35-39`): if `$facts['tpm_version'] == 'tpm1'`, the
  ...
  - Otherwise it `include`s `tpm2::install` and `tpm2::service`, orders
    `Class['tpm2::install'] ~> Class['tpm2::service']` (`init.pp:40-42`), and
    `include`s `tpm2::ownership` only when `$take_ownership` (`init.pp:44-46`).

- **`tpm2::install` (`manifests/install.pp`)** — `assert_private()`
  (`install.pp:3`). Iterates `$tpm2::packages` and declares a `package` for
  each. The per-package `ensure` from data is the literal string
  `package_ensure`, which `regsubst` rewrites to the value of
  `$tpm2::package_ensure` (`install.pp:6-9`) — this is the indirection that lets
  `data/common.yaml` say `ensure: package_ensure` as a placeholder.

- **`tpm2::service` (`manifests/service.pp`)** — `assert_private()`
  (`service.pp:3`). Ensures `Service[$tabrm_service]` is `running`/`enable => true`
  (`service.pp:25-28`). If `$tabrm_options` is set, it writes a systemd drop-in
  via `systemd::dropin_file` (`service.pp:18-22`) that overrides `ExecStart` to
  `/usr/sbin/<service> <joined options>`.

- **`tpm2::ownership` (`manifests/ownership.pp:62-102`)** — the version-dispatch
  entry point. It reads `$facts['tpm2']['tools_version']` and, using
  `versioncmp`, delegates to:
  - `tpm2::ownership::takeownership` when tools **< 4.0.0** (`ownership.pp:73-87`),
    which uses the `tpm2_ownership` type / `tpm2_takeownership` command;
  - `tpm2::ownership::changeauth` when tools **>= 4.0.0** (`ownership.pp:88-98`),
    which uses the `tpm2_changeauth` type / command.
  On tools < 4.0.0, `owner`/`endorsement`/`lockout` of `'ignore'` is a hard
  `fail()` (`ownership.pp:75-77`) — the old tool requires all three values. The
  whole block is a no-op unless both `$facts['tpm2']` and its `tools_version`
  are present (`ownership.pp:72`).

- **`tpm2::ownership::takeownership` (`manifests/ownership/takeownership.pp`)** —
  declares a single `tpm2_ownership { 'tpm2': }` resource (`takeownership.pp:71-80`),
...
- **`tpm2::ownership::changeauth` (`manifests/ownership/changeauth.pp`)** —
  ...
  (e.g. `ownership.pp:66-68`) — auto-generated, host-specific, min length 14
...
- **`tpm2_ownership` type (`lib/puppet/type/tpm2_ownership.rb`)** — namevar
  `name` is validated to be exactly `'tpm2'` (`type/tpm2_ownership.rb:42-53`).
  ...
  (`type/tpm2_ownership.rb:111-117`). `autorequire`s packages `tpm2-tss`,
...
- **`tpm2_takeownership` provider
  ...
  keys (`tpm2_takeownership.rb:73,83,93`). Builds CLI flags per hierarchy
  (`-o/-e/-l`; upper-case variant `-O/-E/-L` when the current auth is already
  set) and appends `-X` when `in_hex` (`tpm2_takeownership.rb:23-55`). **Safety
  ...
  (`tpm2_takeownership.rb:114-124`).

- **`tpm2_changeauth` type (`lib/puppet/type/tpm2_changeauth.rb`)** — namevar
  `name` validated to one of `'owner'`, `'lockout'`, `'endorsement'`
  (`type/tpm2_changeauth.rb:34-44`). Param `auth` (non-empty String,
  `type/tpm2_changeauth.rb:46-56`); property `state` `newvalues(:clear, :set)`.
...
- **`tpm2_changeauth` provider
  ...
  passes `-p <auth>` (`tpm2_changeauth.rb:22-46`). Reads current state from the
  fact's `...TPM2_PT_PERSISTENT.*AuthSet` keys (`tpm2_changeauth.rb:64-68`).

- **`tpm2` fact (`lib/facter/tpm2.rb`, `lib/facter/tpm2/util.rb`)** — `confine`d
  on `tpm2_getcap` being present (`facter/tpm2.rb:6`); nil otherwise. `Util`
  ...
  `facter/tpm2/util.rb:33-39`). It runs `properties-fixed` and
  ...
  raw `tpm2_getcap` output (`facter/tpm2/util.rb:47-92`).

## Gotchas / non-obvious details

- **The property-key prefix differs between the two providers.** The fact
  chooses the prefix (`TPM` vs `TPM2`) by tools version
  (`facter/tpm2/util.rb:33-39`). The `takeownership` provider (used with tools
  < 4) reads `TPM_PT_PERSISTENT` (`tpm2_takeownership.rb:73,83,93`), while the
  `changeauth` provider (used with tools >= 4) reads `TPM2_PT_PERSISTENT`
  (`tpm2_changeauth.rb:64-68`). These line up with the version each provider is
...
- **Ownership does nothing without a usable `tpm2` fact.** `tpm2::ownership`
  ...
  (`ownership.pp:72`), and the fact is `nil` unless `tpm2_getcap` is installed
  and reachable (`facter/tpm2.rb:6`). On tools < 4.0.0 this typically needs
  **two Puppet runs**: one to install `tpm2-tools` so the fact populates, one to
  act on it (noted in `ownership/takeownership.pp:5-7`).
- **The takeownership provider will silently refuse to act if TPM state is
  ...
  out the TPM (`tpm2_takeownership.rb:114-124`). It logs a warning and returns.
- **These types cannot *change* an already-set password**, only set an unset one
  ...
  (`type/tpm2_ownership.rb:24`, `type/tpm2_changeauth.rb:20`) and the class docs.
- **`tpm2::packages` has no default_value in the lookup** (`init.pp:27`) — it is
...
- **The `package_ensure` placeholder in data is a literal string, not a
  ...
  `$tpm2::package_ensure` (`install.pp:6-9`). Setting `ensure:` to a real value
  (e.g. `latest`) in data bypasses that indirection.
- **`$tabrm_options` is explicitly unvalidated** (`init.pp:20-21`) and is joined
  straight into a systemd `ExecStart` line (`service.pp:5-16`) — treat it as a
  command-injection-adjacent surface.
- **tpm1 hosts are a no-op.** A TPM 1.2 device (`$facts['tpm_version'] ==
  'tpm1'`) skips all resources with only a `notify` (`init.pp:35-39`).
...
- **`puppet/systemd`** — `service.pp:18` calls `systemd::dropin_file`, but
...
`env: PUPPET_VERSION: '~> 7'` (`pr_tests.yml:29-30`, the older baseline style).
...
(`Gemfile:36`), `simp-rspec-puppet-facts ~> 4.0.0` (`Gemfile:38`),
`simp-beaker-helpers ~> 2.0.0` (`Gemfile:52`). The Puppet gem is pulled in
**only** through `gem 'puppet', puppet_version` (`Gemfile:29`), where
...
- **Route SIMP package state through the `simp_options::package_ensure` seam.**
  ...
  `init.pp:26`; keep new package resources consistent with it.
...
- **Keep `assert_private()` on `tpm2::install` and `tpm2::service`**
  (`install.pp:3`, `service.pp:3`); they are internal and must be reached via
  `include 'tpm2'`.
- **Dispatch on tools version in `tpm2::ownership`, not in the providers.** The
  ...
  on the `tpm2` fact (`ownership.pp:72-98`); keep that split intact.
- **Preserve the safety guards in the providers** — the unknown-state refusal in
  `tpm2_takeownership.rb:114-124` and the empty-auth validation in the types
